		...
		<?php 
			  ...
			  // Define as variáveis		
				...
				if(isset($_POST["alterar"])) {	
					...
					$evento = $_POST["evento"];
					$local = $_POST["local"];
					$query = mysqli_query($cnn,"UPDATE agenda SET data='$data',hora='$hora',evento='$evento',local='$local' WHERE data = '$data' and hora = '$hora'") or die(mysql_error());
					...
					switch ($_POST["acao"]){
						case "consultar":
							...
							if($result == 0){
								...
								<h2 class="form-signin-heading">Dados do Evento - Excluir</h2>
								<center>
								...
								<input type="text" name="evento" id="evento" class="form-control" placeholder="Informe o Evento..." style ='width:300px;text-align:center' value="<?php echo $evento;?>" /></br>			
								<label for="inputEvento" >Local</label>
								<input type="text" name="local" id="local" class="form-control" placeholder="Informe o Local..." style ='width:500px;text-align:center' value="<?php echo $local;?>" /></br>				
								<button  type="submit" name="alterar" id="alterar" style ='width:200px'/>Confirma Alteração</button>
								<button  type="submit" name="cancelar" id="cancelar" style ='width:200px'/>Cancela Alteração</button>
								</center>
							  </form>	
