		...
		<?php 
			  ...
			  // Define as variáveis		
				...
				if(isset($_POST["excluir"])) {	
					$data = $_POST["data"];
					$hora = $_POST["hora"];
					$query = mysqli_query($cnn,"DELETE FROM agenda WHERE data = '$data' and hora = '$hora'") or die(mysql_error());      
						echo "<script> alert('Evento Excluido!'); </script>";
					...
					switch ($_POST["acao"]){
						case "consultar":
							...
							if($result == 0){
								...
								<button  type="submit" name="excluir" id="excluir" style ='width:200px'/>Confirma Exclusão</button>
								<button  type="submit" name="cancelar" id="cancelar" style ='width:200px'/>Cancela Exclusão</button>
								</center>
							  </form>	
